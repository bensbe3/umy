...
const DEFAULT_IMAGES = [
  { src: timelineImages['1.webp'], alt: 'Gallery image 1' },
  { src: timelineImages['2.webp'], alt: 'Gallery image 2' },
  { src: timelineImages['3.webp'], alt: 'Gallery image 3' },
  { src: timelineImages['4.webp'], alt: 'Gallery image 4' },
  { src: timelineImages['5.webp'], alt: 'Gallery image 5' },
  { src: timelineImages['6.webp'], alt: 'Gallery image 6' },
  { src: timelineImages['7.webp'], alt: 'Gallery image 7' },
  { src: timelineImages['8.webp'], alt: 'Gallery image 8' },
  { src: timelineImages['9.webp'], alt: 'Gallery image 9' },
  { src: timelineImages['10.webp'], alt: 'Gallery image 10' },
  { src: timelineImages['11.webp'], alt: 'Gallery image 11' },
  { src: timelineImages['12.webp'], alt: 'Gallery image 12' },
  { src: timelineImages['13.webp'], alt: 'Gallery image 13' }
].filter(img => img.src); // Filter out any missing images
...
function buildItems(pool: (string | ImageItem)[], seg: number): GridItem[] {
...
function DomeGallery({
  ...
  // Optimized drag handler with throttling for better performance
  const throttledApplyTransform = useMemo(
    () => {
      let lastCall = 0;
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        if (cur.x !== nextX || cur.y !== nextY) {
          rotationRef.current = { x: nextX, y: nextY };
          // Use throttled transform for better performance during drag
          throttledApplyTransform(nextX, nextY);
        }
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (wantsResize) {
      const onFirstEnd = (ev: TransitionEvent) => {
        ...
        overlay.addEventListener('transitionend', cleanupSecond as any, {
          once: true
        });
      };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="overlay-gradient-mask" />
...
// Memoized Gallery Page for better performance
export const GalleryPage = memo(function GalleryPage() {
  // Check if images are available
  const hasImages = DEFAULT_IMAGES.length > 0 && DEFAULT_IMAGES[0]?.src;
